...
| 层 | 频率 | 说明 |
|---|---|---|
| 几何（圈/线/路径位置） | **0.25s tick（4Hz）** | 慢移动目标 4Hz 视觉无差，不每帧 |
| 文字（弹种/时间/距离/速度标签） | **0.25s 同 tick，只在值变时设** | 倒计时一位小数 4Hz；速度标签变化极稀 |
| 出现/消失 | 事件驱动 | 任务进炮管/落地/取消时增删，不轮询 |

每帧只剩一个节流检查，几乎零成本。不做每帧 LineRenderer 更新（慢目标 0.25s 走 2m，10km 地图上不可见）。
