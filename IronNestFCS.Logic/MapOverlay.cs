...
public class MapOverlay
{
    ...
    private const float Ring2Scale = 1.03f;        // 二次描圈半径系数(圆规描两遍没对齐)
    private const float Ring2Thickness = 0.006f;   // 二次描圈线宽(更细)
    private const float FireExtLen = 0.6f;         // 火线延长线长度(测量基准线穿过目标)
    private const float FireExtThickness = 0.005f; // 延长线宽(细虚线)
    ...
    private sealed class Slot
    {
        ...
        public readonly List<GameObject> ring = new();   // 毁伤圈 24 段(手绘缺口)
        public readonly List<GameObject> ring2 = new();  // 二次描圈(部分弧段, 圆规描两遍)
        ...
        public GameObject? fireExt;                      // 火线延长线(细虚线)
    ...
    private static IEnumerable<GameObject> Collect(Slot s)
    {
        ...
        foreach (var g in s.ring2) yield return g;
        ...
        if (s.fireExt != null) yield return s.fireExt;
    ...
    private void UpdateSlot(Slot s, ArtilleryTask t)
    {
        ...
        // 毁伤圈: 圆规描出的测量圈——轻微不完美 + 两处落笔/抬笔缺口 + 二次描圈(部分弧段, 没对齐)
        ...
        if (r > 0f)
        {
            ...
            for (int i = 0; i < RingSegments; i++)
            {
                ...
                bool gap = i == 5 || i == 6 || i == 17 || i == 18;   // 圆规落笔/抬笔缺口
                if (l.gameObject.activeSelf == gap) l.gameObject.SetActive(!gap);
                // 二次描圈: 半径略大 3%、更细、只描约 2/3 圈且错开相位
                float a2 = (i + 0.5f) * 2f * Mathf.PI / RingSegments - 0.3f;
                float a2n = (i + 1.5f) * 2f * Mathf.PI / RingSegments - 0.3f;
                var l2 = s.ring2[i].GetComponent<Il2CppShapes.Line>();
                SetLine(l2, new Vector3(Mathf.Cos(a2) * r * Ring2Scale, Mathf.Sin(a2) * r * Ring2Scale, 0f),
                           new Vector3(Mathf.Cos(a2n) * r * Ring2Scale, Mathf.Sin(a2n) * r * Ring2Scale, 0f));
                bool arc2 = i >= 4 && i <= 19;
                if (l2.gameObject.activeSelf == arc2) l2.gameObject.SetActive(!arc2);
            }
        ...
        else if (s.ring.Count > 0)
        {
            ...
            foreach (var g in s.ring2) if (g.activeSelf) g.SetActive(false);
        ...
        // 延长线: 细虚线从落点继续延伸——手绘测量基准线不裁切, 直尺划过去
        if (s.fireExt == null)
        {
            s.fireExt = MakeLine(s.root.transform, "FCS_OverlayFireExt", InkRed, FireExtThickness, Vector3.zero);
            tracked.Add(s.fireExt);
        }
        var fe = s.fireExt.GetComponent<Il2CppShapes.Line>();
        if (fdir.magnitude > 0.001f)
        {
            SetLine(fe, Vector3.zero, fdir.normalized * FireExtLen);
            fe.Dashed = true;
            if (!fe.gameObject.activeSelf) fe.gameObject.SetActive(true);
        }
        else if (fe.gameObject.activeSelf) fe.gameObject.SetActive(false);
    ...
    private void EnsureRing(Slot s)
    {
        ...
        for (int i = 0; i < RingSegments; i++)
        {
            var g = MakeLine(s.root.transform, "FCS_OverlayRing2", InkRed, Ring2Thickness, Vector3.zero);
            s.ring2.Add(g);
            tracked.Add(g);
        }
    ...
    /// <summary>手绘圆轻微不完美(±2%): "模拟手工测量而非手抖"——圆规痕迹主要由缺口/二次描圈承担。</summary>
    private static readonly float[] RingJitter = BuildJitter(RingSegments, 0.02f);
